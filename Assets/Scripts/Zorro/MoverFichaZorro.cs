...
public class MoverFichaZorro : MonoBehaviour
{
    ...
    //Coloca los spawneadores de paseadores en la direccion que tu le pidas
    ...
    public bool poderMoverFichaZorro = true;
    ...
    void Update()
    {
        ...
        if((turnoAnterior + 2) == gameManager.turnosRegistrados){
            poderMoverFichaZorro = false;
        }
        else{
            poderMoverFichaZorro = true;
        }
    ...
    private void LimitesPaseador(){
        ...
        if (posX >= 0 && posX <= 6 && posY >= 0 && posY <= 6) {
            ...
            for (int y = posY - 1; y >= 0; y--) {
                ...
                if(y == posYAnterior && !poderMoverFichaZorro){
                ...
                foreach (Collider collider in colliders) {
                    ...
                    else if (collider.gameObject.CompareTag("Pato") || collider.gameObject.CompareTag("Pavo")) {
                        Debug.Log("No Toco hierva");
                        colisionConClonArriba = true;
                        break;
                    }
                    else if(collider.gameObject.CompareTag("Cazador") || collider.gameObject.CompareTag("Pino") || collider.gameObject.CompareTag("Leñador") || collider.gameObject.CompareTag("Zorro") || collider.gameObject.CompareTag("Oso") || collider.gameObject.CompareTag("Arbol")){
                        Destroy(listaPaseadoresArriba[listaPaseadoresArriba.Count - 1]);
                        colisionConClonArriba = true;
                        break;
                    }
        ...
        if(posX >= 0 && posX <= 6 && posY >= 0 && posY <= 6){
            ...
            for (int x = posX + 1; x <= 6; x++){
                ...
                if(x == posXAnterior && !poderMoverFichaZorro){
    ...
    private void rescatarFichaZorro(){
