...
public class MoverFichaPato : MonoBehaviour
{
    ...
    public bool poderMoverFichaPato = true;

    public int turnoAnterior = 0;

    private GameManager gameManager;

    public bool fichaDisponiblePato = true;
    ...
    {   
        if((turnoAnterior + 1) == gameManager.turnosRegistrados){
            poderMoverFichaPato = false;
        }
        else{
            poderMoverFichaPato = true;
        }
    ...
    private void OnMouseDown()
    {
        if(gameManager.getPoderClickear()){
            ...
            else if(!poderMoverFichaPato){
                Debug.Log("No se puede mover una loseta neutral dos veces seguidas");
            }
            else{
